...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AuthService,
        { provide: AUTH_REPOSITORY_TOKEN, useValue: authRepository },
  ...
  it('rechaza el login cuando el usuario está inactivo', async () => {
    authRepository.findByEmailWithPassword.mockResolvedValue({
      ...usuarioActivo,
      isActive: false,
    });
    bcryptCompare.mockResolvedValue(true);
    ...
    ).rejects.toThrow(new UnauthorizedException('El usuario está inactivo'));
  });
});
