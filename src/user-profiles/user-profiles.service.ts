...
export class UserProfilesService {
  ...
  constructor(@Inject(USER_PROFILE_REPOSITORY_TOKEN) private readonly userProfileRepository: IUserProfileRepository) {}
  ...
  async create(createUserProfileDto: CreateUserProfileDto): Promise<UserProfile> {
    ...
    const countryCode = this.normalizeCountryCode(createUserProfileDto.countryCode);

    return await this.userProfileRepository.save({ ...createUserProfileDto, countryCode });
  ...
  async update(id: number, updateUserProfileDto: UpdateUserProfileDto): Promise<UserProfile> {
    const countryCode = this.normalizeCountryCode(updateUserProfileDto.countryCode);
    const updated = await this.userProfileRepository.update(id, { ...updateUserProfileDto, countryCode });
